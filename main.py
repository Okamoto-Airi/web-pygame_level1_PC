...
from pygame.locals import QUIT, KEYDOWN, K_SPACE, K_q, K_r, K_a, K_d, K_UP
...
async def main():
    ...
    pygame.display.set_caption("Pygame - Stage 1")  # ウィンドウタイトルの設定
    ...
    Ufo.containers = group  # UFOスプライトの所属グループ
    ...
    while True:
        ...
        # 衝突判定（ゲームプレイ中かつUFOが存在する場合のみ）
        if game_status == PLAY and ufo is not None:
            collision_detection(majo, ufo, beam_g, bomb_g)  # 衝突判定処理
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                # プレイ中に上キーでビーム発射
                if event.key == K_UP and game_status == PLAY:
        ...
        if pressed_keys[K_a]:  # Aキー(左)
            majo.move_left()  # 魔女を左に移動
        elif pressed_keys[K_d]:  # Dキー(右)
            majo.move_right()  # 魔女を右に移動
